...
(function($, window, document, undefined) {
	...
	Picker.prototype.calcAngle = function(c) {
		...
		//鐩村緞鐨勬暣鍊嶆暟閮ㄥ垎鐩存帴涔樹互 180
		c = Math.abs(c); //鍙畻瑙掑害涓嶅叧蹇冩鍚﹀€�
	...
	Picker.prototype.startInertiaScroll = function(event) {
		 ...
		 */
		var nowTime = event.timeStamp || Date.now();
		var v = (point.pageY - self.lastMoveStart) / (nowTime - self.lastMoveTime); //鏈€鍚庝竴娈垫椂闂存墜鎸囧垝鍔ㄩ€熷害  
		var dir = v > 0 ? -1 : 1; //鍔犻€熷害鏂瑰悜  
		var deceleration = dir * 0.0006 * -1;
		var duration = Math.abs(v / deceleration); // 閫熷害娑堝噺鑷�0鎵€闇€鏃堕棿  
		var dist = v * duration / 2; //鏈€缁堢Щ鍔ㄥ灏� 
